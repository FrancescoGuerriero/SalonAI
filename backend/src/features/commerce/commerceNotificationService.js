...
export async function notifyOrderPaid(orderId) {
  ...
  const body = `Hi ${name}, payment of Ã‚£${amount} for SalonAI order ${order.orderNumber} has been received. Your order is now being prepared for ${fulfilment}.`;
  ...
  return sendTransactionalNotification({
    ...
    text: body,
    html: `<p>Hi ${name},</p><p>Payment of <strong>Ã‚£${amount}</strong> for order <strong>${order.orderNumber}</strong> has been received.</p><p>Your order is now being prepared for ${fulfilment}.</p>`,
    whatsapp: {
      body,
...
export async function notifyOrderRefunded(
  ...
  const body = `Hi ${name}, a ${refundLabel} of Ã‚£${refundedAmount} has been recorded for SalonAI order ${order.orderNumber}. Your bank or card provider may take additional time to display the funds.`;
  ...
  return sendTransactionalNotification({
    ...
    text: body,
    html: `<p>Hi ${name},</p><p>A ${refundLabel} of <strong>Ã‚£${refundedAmount}</strong> has been recorded for order <strong>${order.orderNumber}</strong>.</p><p>Your bank or card provider may take additional time to display the funds.</p>`,
    whatsapp: {
      body,
