...
try {
    ...
    Check (-not [bool]$InputExample.sshTransportConfigured) "SSH transport is not pre-confirmed"
...
Check ($EnvironmentScript.Contains('AI_SERVICE_KEY=$AiServiceKey')) "Backend AI-service key generation"
Check ($EnvironmentScript.Contains('SERVICE_KEY=$AiServiceKey')) "AI-service shared-key generation"
...
Check ($EnvironmentScript -notmatch "Write-Host.*(MongoPassword|JwtSecret|AiServiceKey|GrafanaPassword)") "Generated secrets are not printed"
...
Check ($GitHubScript -match "PRODUCTION_HOST") "Production host secret lookup"
Check ($GitHubScript -match "PRODUCTION_USER") "Production user secret lookup"
Check ($GitHubScript -match "PRODUCTION_SSH_KEY") "Production SSH key secret lookup"
Check ($GitHubScript -match "PRODUCTION_KNOWN_HOSTS") "Production known-hosts secret lookup"
