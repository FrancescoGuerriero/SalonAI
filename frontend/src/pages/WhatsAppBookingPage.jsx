...
export default function WhatsAppBookingPage() {
  ...
  const currentServiceWindowOpen =
    serviceWindowOpen(
      conversation
        ?.lastInboundAt
    );
  ...
  async function checkAvailability() {
    if (
      ...
      setError(
        "Choose a service, stylist, and date before checking availability."
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        setNotice(
          "No available times were found for that day."
        );
      }
    } catch (
      requestError
  ...
  async function saveBooking(
    ...
    try {
      const result =
        await updateWhatsAppBookingSession(
          selectedId,
          booking
        );
      ...
      setNotice(
        result.message ||
          "Booking details are ready to confirm."
      );
      ...
      setError(
        requestMessage(
          requestError,
          "Unable to save the booking details."
        )
      );
    } finally {
      setSaving(false);
  ...
  async function confirmBooking() {
    ...
    try {
      ...
      setError(
        requestMessage(
          requestError,
          "Unable to confirm the WhatsApp booking."
        )
      );
    } finally {
      setSaving(false);
  ...
  async function sendMessage(
    ...
    const contentSid =
      replyContentSid.trim();

    if (
      currentServiceWindowOpen &&
      !body
    ) {
      return;
    }
    ...
    if (
      ...
      setError(
        "This conversation is outside the 24-hour WhatsApp customer service window. Select an approved Content SID."
      );
      return;
    }
  ...
  async function sendOneOff(
    ...
    try {
      const result =
        await sendOneOffWhatsAppMessage({
          ...oneOff,
          contentVariables:
            oneOff
              .contentSid
      ...
      setError(
        requestMessage(
          requestError,
          "Unable to send the one-off WhatsApp message."
        )
      );
    } finally {
      setOneOffSending(
      ...
      <div className="whatsapp-workspace-inner">
        <header className="whatsapp-page-header">
          <div>
            ...
            <p>
              Review incoming requests, send one-off messages, verify live appointment slots,
        ...
        <section
          ...
          aria-label="WhatsApp booking summary"
        >
          <article>
            <span>
              <Inbox size={19} />
        ...
        <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4">
            ...
            <h2 className="mt-1 text-xl font-bold text-slate-900">
              Start or continue a WhatsApp conversation
            </h2>
            <p className="mt-2 text-sm text-slate-600">
              Free-form text is allowed only inside an open 24-hour customer service window.
              Otherwise use an approved Twilio Content SID. Confirm customer consent before sending.
            </p>
          </div>
          ...
          >
            ...
            <label className="md:col-span-2 text-sm font-semibold text-slate-700">
              ...
              <textarea
                ...
                placeholder="Free-form message for an open service window, or an optional audit preview when using a template."
                className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2"
              />
            </label>

            <label className="text-sm font-semibold text-slate-700">
              Approved Content SID
              <input
                value={oneOff.contentSid}
                onChange={(event) =>
                  setOneOff(
                    (current) => ({
                      ...current,
            ...
            <label className="text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                placeholder='{"1":"Francesco","2":"10:30"}'
            ...
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={oneOff.consentConfirmed}
                onChange={(event) =>
                  setOneOff(
                    (current) => ({
                      ...current,
            ...
            <div className="md:col-span-2">
              <button
                ...
                disabled={
                  oneOffSending ||
                  !oneOff.to.trim() ||
                  !oneOff.consentConfirmed ||
                  (!oneOff.body.trim() &&
                    !oneOff.contentSid.trim())
                }
              >
                {oneOffSending ? (
                  <Loader2
        ...
        <section className="whatsapp-console">
            ...
            <div className="whatsapp-conversation-list">
                ...
                rows.map(
                  (item) => (
                    <button
                      type="button"
                      key={item._id}
                      className={`whatsapp-conversation-row${
                        selectedId === item._id
                          ? " is-selected"
                          : ""
                      }`}
                    ...
                    >
                      <span className="whatsapp-customer-avatar">
                        {customerName(
                          item
                        )
                          .charAt(0)
                      ...
                      <span className="whatsapp-conversation-copy">
                        ...
                        <span>
                          <em
                            className={`whatsapp-status is-${item.status}`}
                          >
                            {STATUS_LABELS[
                              item.status
                            ] ||
              ...
              !rows.length ? (
                <div className="whatsapp-empty">
                  ...
                  <p>
                    Incoming verified WhatsApp messages will appear here.
                  </p>
                </div>
              ) : null}
            </div>
          ...
          <div className="whatsapp-detail">
            {!selectedId ? (
              <div className="whatsapp-empty large">
                <MessageCircle
                  size={36}
                />
                <strong>
                ...
                <p>
                  Choose a conversation to review messages and prepare a booking.
                </p>
              </div>
            ) : detailLoading ? (
              <div className="whatsapp-loading large">
                <Loader2 className="app-spin" />
              ...
              <>
                <header className="whatsapp-detail-header">
                  <div>
                    <span className="whatsapp-customer-avatar large">
                      {customerName(
                        conversation
                      )
                        .charAt(0)
                ...
                <div className="whatsapp-detail-grid">
                  ...
                  >
                    <div className="mb-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
                      {currentServiceWindowOpen ? (
                        <p className="font-semibold text-emerald-700">
                          24-hour customer service window open — free-form replies are allowed.
                        </p>
                      ) : (
                        <p className="font-semibold text-amber-700">
                          Customer service window closed — use an approved Twilio Content SID.
                        </p>
                      )}
                    </div>
                      ...
                      {messages.map(
                        (
                          ...
                          <article
                            ...
                            className={`whatsapp-message is-${message.direction}`}
                          >
                            <p>
                              {message.body}
                            </p>
                    ...
                    <form
                      className="whatsapp-reply-form"
                      onSubmit={sendMessage}
                    >
                      <label
                        htmlFor="whatsapp-reply"
                        className="sr-only"
                      >
                        Reply on WhatsApp
                      </label>
                      ...
                      <textarea
                        ...
                        placeholder={
                          currentServiceWindowOpen
                            ? "Write a clear booking reply…"
                            : "Optional audit preview for the approved template"
                        }
                      />
                      ...
                      {!currentServiceWindowOpen ? (
                        <div className="grid w-full gap-2 md:grid-cols-2">
                          <input
                            ...
                            placeholder="Approved Content SID (HX...)"
                  ...
                  <section className="whatsapp-booking-card">
                    ...
                    {alreadyBooked ? (
                      <div className="whatsapp-booked-state">
                        <CheckCircle2
                          size={24}
                        />
                      ...
                      <form
                        ...
                        className="whatsapp-booking-form"
                      >
                        <label>
                          Customer name
                          <input
                        ...
                        {slots.length ? (
                          <fieldset className="whatsapp-slots">
                            <legend>
                              Available times
                            </legend>
